create or replace type body xxdoo_db_scheme is
  ...
  member procedure ctable(p_table_name  varchar2, 
    ...
    --l_all_constraints xxdoo_db_constraints;
    --
    /*cursor l_constraints_cur is
      select value(c)
      from   talbe(self.table_list) t,
             table(t.constraints)   c;*/
             
  begin
    --
    /*open l_constraints_cur;
    fetch bulk collect into l_all_constraints;
...
--          self.table_list(pos).constraints(c).db_table := self.table_list(pos).db_table;
  ...
  begin
    ...
    --
    --prepare_constraints(pos);
    --
    self.table_list(pos).prepare_index;
    --
  exception
    when others then
      xxdoo_db_utils_pkg.fix_exception;
  ...
  begin
    ...
    --
    self.table_list(p_tab_pos).prepare_attributes;
  ...
  begin
    ...
    --
    self.table_list(p_tab_pos).final_prepare_attributes(self);
  ...
  begin
    ...
    using (select t.id           id,
                  ...
                  t.load_method  load_method,
                  t.put_method   put_method,
                  sysdate        event_date
           from   table(self.table_list) t
          ) u
    on    (s.id = u.id)
    when matched then
      update set    
        ...
        s.last_update_date = u.event_date,
        s.load_method = u.load_method,
        s.put_method  = u.put_method
    when not matched then
      insert(s.id,
             s.scheme_id,
             s.version,
